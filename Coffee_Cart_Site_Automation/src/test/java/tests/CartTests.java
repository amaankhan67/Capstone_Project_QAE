...
public class CartTests {
	...
	public void verifyItemPresenceInCart(String itemName) {
	...
	public Object[][] itemDataProvider() {
		return ExcelReader.getTestData("itemList");
	}
	
	@Test(description = "Verify total amount for a single item in cart",
			dataProvider = "singleItemDataProvider")
	public void verifySingleItemTotalPrice(String itemName, String expectedPrice) {
		menuPage.addItemToCart(itemName);
		menuPage.clickCartButton();
	...
	@DataProvider(name = "singleItemDataProvider")
    ...
    @Test(description = "Verify that item quantity cannot go below zero")
    public void verifyQuantityCannotGoBelowZero() {
        String itemName = "Espresso";
        menuPage.addItemToCart(itemName);
        menuPage.clickCartButton();

        cartPage.decreaseItemQuantity(itemName); // now quantity 0
        cartPage.decreaseItemQuantity(itemName); // should stay 0

        int quantity = cartPage.getItemQuantity(itemName); 
        Assert.assertTrue(quantity >= 0, "Quantity went below zero");
        logger.info("Test Passed: Quantity cannot go below zero");
    }
